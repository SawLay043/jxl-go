...
- [ ] Task: Create reproduction/verification tests for `writeSRGB` in `core/png_writer_test.go`
    - [ ] Add a test case that checks if `writeSRGB` correctly handles different rendering intents.
    - [ ] Verify that the current hardcoded implementation always produces the same output regardless of input.
...
- [ ] Task: Refactor `writeSRGB` to use `*JXLImage` and dynamic CRC
    - [ ] Rename parameter `_` to `jxlImage`.
    - [ ] Implement rendering intent retrieval from `jxlImage.imageHeader.ColourEncoding.RenderingIntent`.
    - [ ] Implement dynamic CRC32 calculation for the `sRGB` chunk.
    - [ ] Standardize the chunk writing process (Length, Type, Data, CRC).
- [ ] Task: Verify `writeSRGB` changes
    - [ ] Run tests in `core/` and ensure success for `writeSRGB`.
    - [ ] Check coverage for `core/png_writer.go`.
...
- [ ] Task: Update `displayBuffer` and `displayBuffers` to use all parameters
    - [ ] Rename `_` to `label`.
